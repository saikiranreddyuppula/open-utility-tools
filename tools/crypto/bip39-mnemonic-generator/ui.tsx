...
/* Synchronous SHA-256 (FIPS 180-4) over a byte array, returns 32 bytes. */
function sha256Sync(message: Uint8Array): Uint8Array {
  ...
  buf.set(message);
  buf[ml] = 0x80;
  // big-endian 64-bit length; high 32 bits assumed 0 for typical inputs
  ...
  for (let off = 0; off < total; off += 64) {
    for (let i = 0; i < 16; i++) {
      const p = off + i * 4;
      w[i] =
        ((buf[p] ?? 0) << 24) | ((buf[p + 1] ?? 0) << 16) | ((buf[p + 2] ?? 0) << 8) | (buf[p + 3] ?? 0);
    }
    for (let i = 16; i < 64; i++) {
      const w15 = w[i - 15] ?? 0;
      const w2 = w[i - 2] ?? 0;
...
function bytesToBits(bytes: Uint8Array, take: number): string {
  let bits = '';
  for (let i = 0; i < bytes.length && bits.length < take; i++) {
    bits += (bytes[i] ?? 0).toString(2).padStart(8, '0');
  }
  return bits.slice(0, take);
}

export default function Bip39MnemonicGenerator() {
  const [count, setCount] = useState<WordCount>('12');

  const generate = () => {
    const entBits = ENTROPY_BITS[count];
    const entBytes = entBits / 8;
    const entropy = webcrypto.getRandomValues(new Uint8Array(entBytes));
    const hash = sha256Sync(entropy);
    const checksumBits = entBits / 32;
    const fullBits = bytesToBits(entropy, entBits) + bytesToBits(hash, checksumBits);
    const words: string[] = [];
    for (let i = 0; i + 11 <= fullBits.length; i += 11) {
      const idx = parseInt(fullBits.slice(i, i + 11), 2);
      words.push(WORDLIST[idx] ?? 'abandon');
    }
    return words.join(' ');
  };
  ...
  return (
    <GeneratorList
      ...
      defaultCount={1}
      maxCount={20}
      downloadName="mnemonics.txt"
      label="Mnemonics"
      options={
        <Field label="Words">
          <Select value={count} onValueChange={(v) => setCount(v as WordCount)}>
