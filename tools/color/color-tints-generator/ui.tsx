...
export default function ColorTintsGeneratorTool() {
  ...
  const tints = useMemo(() => {
    ...
    // i = 0 is the original color; i = steps approaches white.
    ...
    for (let i = 1; i <= steps; i++) {
      ...
      if (method === 'srgb') {
        ...
        // Raise lightness toward 1 (white) while holding hue and saturation.
        c = hslToRgb({ h: baseHsl.h, s: baseHsl.s, l: baseHsl.l + (1 - baseHsl.l) * t });
      }
      out.push({ hex: rgbToHex(c), rgb: rgbStr(c) });
    }
    return out;
  }, [baseRgb, steps, method]);

  const cssVars = useMemo(
    () =>
      ':root {\n' +
      tints.map((t, i) => `  --tint-${i * 100}: ${t.hex};`).join('\n') +
      '\n}',
    [tints],
  );
  ...
  return (
    <Panel>
      <PanelHeader title="Color Tints Generator" />
      <div className="flex flex-col gap-4 p-4">
        <OptionsBar>
          ...
          <Field label="Method">
            <Tabs value={method} onValueChange={(v) => setMethod(v as Method)}>
              <TabsList>
                <TabsTrigger value="srgb">sRGB mix</TabsTrigger>
                <TabsTrigger value="hsl">HSL lightness</TabsTrigger>
              </TabsList>
            </Tabs>
          </Field>
        </OptionsBar>
        ...
        {tints.length > 0 ? (
          <>
            ...
            <StatBar
              items={[`${tints.length} swatches`, method === 'srgb' ? 'sRGB toward white' : 'HSL lightness']}
            />
