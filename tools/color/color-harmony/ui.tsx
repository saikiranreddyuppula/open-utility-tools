'use client';

import { useMemo, useState } from 'react';
import { CopyButton } from '@/components/tools/copy-button';
import { ErrorBanner } from '@/components/tools/error-banner';
import { Field, OptionsBar, Panel, PanelHeader, StatBar } from '@/components/tools/panel';
import { Input } from '@/components/ui/input';
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/components/ui/select';

type Scheme =
  | 'complementary'
  | 'analogous'
  | 'triadic'
  | 'tetradic'
  | 'split'
  | 'monochromatic';

interface Hsl {
  h: number;
  s: number;
  l: number;
}

function clampByte(n: number): number {
  return Math.max(0, Math.min(255, Math.round(n)));
}

function toHexByte(n: number): string {
  return clampByte(n).toString(16).padStart(2, '0');
}

function normalizeHue(h: number): number {
  let x = h % 360;
  if (x < 0) x += 360;
  return x;
}

function hslToRgb(h: number, s: number, l: number): [number, number, number] {
  const c = (1 - Math.abs(2 * l - 1)) * s;
  const hp = normalizeHue(h) / 60;
  const x = c * (1 - Math.abs((hp % 2) - 1));
  let r1 = 0;
  let g1 = 0;
  let b1 = 0;
  if (hp < 1) {
    r1 = c;
    g1 = x;
  } else if (hp < 2) {
    r1 = x;
    g1 = c;
  } else if (hp < 3) {
    g1 = c;
    b1 = x;
  } else if (hp < 4) {
    g1 = x;
    b1 = c;
  } else if (hp < 5) {
    r1 = x;
    b1 = c;
  } else {
    r1 = c;
    b1 = x;
  }
  const m = l - c / 2;
  return [(r1 + m) * 255, (g1 + m) * 255, (b1 + m) * 255];
}

function hslToHex(h: number, s: number, l: number): string {
  const [r, g, b] = hslToRgb(h, s, l);
  return `#${toHexByte(r)}${toHexByte(g)}${toHexByte(b)}`;
}

function rgbToHsl(r: number, g: number, b: number): Hsl {
  const rn = r / 255;
  const gn = g / 255;
  const bn = b / 255;
  const max = Math.max(rn, gn, bn);
  const min = Math.min(rn, gn, bn);
  const d = max - min;
  let h = 0;
  if (d !== 0) {
    if (max === rn) h = ((gn - bn) / d) % 6;
    else if (max === gn) h = (bn - rn) / d + 2;
    else h = (rn - gn) / d + 4;
    h *= 60;
  }
  const l = (max + min) / 2;
  const s = d === 0 ? 0 : d / (1 - Math.abs(2 * l - 1));
  return { h: normalizeHue(h), s, l };
}

/** Parse #rgb / #rrggbb / rgb()/hsl() into HSL. Throws on invalid. */
function parseColor(input: string): Hsl {
  const s = input.trim().toLowerCase();
  if (!s) throw new Error('Enter a base color.');

  // HEX
  const hexMatch = /^#?([0-9a-f]{3}|[0-9a-f]{6})$/.exec(s);
  if (hexMatch && hexMatch[1] !== undefined) {
    let hex = hexMatch[1];
    if (hex.length === 3) {
      hex = hex
        .split('')
        .map((c) => c + c)
        .join('');
    }
    const r = parseInt(hex.slice(0, 2), 16);
    const g = parseInt(hex.slice(2, 4), 16);
    const b = parseInt(hex.slice(4, 6), 16);
    return rgbToHsl(r, g, b);
  }

  // rgb()
  const rgbMatch = /^rgba?\s*\(([^)]*)\)$/.exec(s);
  if (rgbMatch && rgbMatch[1] !== undefined) {
    const parts = rgbMatch[1]
      .split(/[\s,/]+/)
      .map((p) => p.trim())
      .filter((p) => p.length > 0);
    const r = Number(parts[0]);
    const g = Number(parts[1]);
    const b = Number(parts[2]);
    if (![r, g, b].every(Number.isFinite)) throw new Error('Invalid rgb() value.');
    return rgbToHsl(r, g, b);
  }

  // hsl()
  const hslMatch = /^hsla?\s*\(([^)]*)\)$/.exec(s);
  if (hslMatch && hslMatch[1] !== undefined) {
    const parts = hslMatch[1]
      .split(/[\s,/]+/)
      .map((p) => p.trim())
      .filter((p) => p.length > 0);
    const h = Number((parts[0] ?? '').replace(/deg$/, ''));
    const sat = Number((parts[1] ?? '').replace(/%$/, '')) / 100;
    const lig = Number((parts[2] ?? '').replace(/%$/, '')) / 100;
    if (![h, sat, lig].every(Number.isFinite)) throw new Error('Invalid hsl() value.');
    return { h: normalizeHue(h), s: Math.max(0, Math.min(1, sat)), l: Math.max(0, Math.min(1, lig)) };
  }

  throw new Error('Unrecognized color. Use HEX (#3b82f6), rgb(), or hsl().');
}

const SCHEME_LABELS: Record<Scheme, string> = {
  complementary: 'Complementary',
  analogous: 'Analogous',
  triadic: 'Triadic',
  tetradic: 'Tetradic (square)',
  split: 'Split-complementary',
  monochromatic: 'Monochromatic',
};

function buildPalette(base: Hsl, scheme: Scheme): Hsl[] {
  const { h, s, l } = base;
  switch (scheme) {
    case 'complementary':
      return [base, { h: normalizeHue(h + 180), s, l }];
    case 'analogous':
      return [
        { h: normalizeHue(h - 30), s, l },
        base,
        { h: normalizeHue(h + 30), s, l },
      ];
    case 'triadic':
      return [
        base,
        { h: normalizeHue(h + 120), s, l },
        { h: normalizeHue(h + 240), s, l },
      ];
    case 'tetradic':
      return [
        base,
        { h: normalizeHue(h + 90), s, l },
        { h: normalizeHue(h + 180), s, l },
        { h: normalizeHue(h + 270), s, l },
      ];
    case 'split':
      return [
        base,
        { h: normalizeHue(h + 150), s, l },
        { h: normalizeHue(h + 210), s, l },
      ];
    case 'monochromatic': {
      const steps = [0.2, 0.35, 0.5, 0.65, 0.8];
      return steps.map((ll) => ({ h, s, l: ll }));
    }
    default:
      return [base];
  }
}

function hslText(c: Hsl): string {
  return `hsl(${Math.round(c.h)}, ${Math.round(c.s * 100)}%, ${Math.round(c.l * 100)}%)`;
}

function Swatch({ color }: { color: Hsl }) {
  const hex = hslToHex(color.h, color.s, color.l);
  const textDark = color.l > 0.55;
  return (
    <div className="overflow-hidden rounded-lg border">
      <div
        className="flex h-24 items-end justify-between p-2"
        style={{ backgroundColor: hex }}
      >
        <span
          className={`font-mono text-xs ${textDark ? 'text-black/70' : 'text-white/80'}`}
        >
          {hex}
        </span>
        <CopyButton value={hex} />
      </div>
      <div className="flex items-center justify-between gap-2 px-2 py-1.5 text-xs text-muted-foreground">
        <span className="truncate font-mono">{hslText(color)}</span>
        <CopyButton value={hslText(color)} />
      </div>
    </div>
  );
}

export default function ColorHarmonyTool() {
  const [value, setValue] = useState('#3b82f6');
  const [scheme, setScheme] = useState<Scheme>('complementary');

  const result = useMemo(() => {
    try {
      const base = parseColor(value);
      return { palette: buildPalette(base, scheme), error: null as string | null };
    } catch (e) {
      return {
        palette: [] as Hsl[],
        error: e instanceof Error ? e.message : 'Invalid color',
      };
    }
  }, [value, scheme]);

  const allHex = result.palette.map((c) => hslToHex(c.h, c.s, c.l)).join('\n');

  return (
    <Panel>
      <PanelHeader title="Color Harmony / Palette Builder" />
      <OptionsBar>
        <Field label="Base color" className="min-w-[220px]">
          <Input
            value={value}
            onChange={(e) => setValue(e.target.value)}
            placeholder="#3b82f6 or rgb(59,130,246)"
            spellCheck={false}
          />
        </Field>
        <Field label="Scheme">
          <Select value={scheme} onValueChange={(v) => setScheme(v as Scheme)}>
            <SelectTrigger className="w-[220px]">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              {(Object.keys(SCHEME_LABELS) as Scheme[]).map((k) => (
                <SelectItem key={k} value={k}>
                  {SCHEME_LABELS[k]}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
        </Field>
      </OptionsBar>

      <ErrorBanner error={result.error} />

      {result.palette.length > 0 && (
        <>
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4">
            {result.palette.map((c, i) => (
              <Swatch key={`${i}-${c.h}-${c.l}`} color={c} />
            ))}
          </div>
          <StatBar
            items={[
              `${result.palette.length} colors`,
              SCHEME_LABELS[scheme],
            ]}
          />
          <div className="flex justify-end">
            <CopyButton value={allHex} />
          </div>
        </>
      )}
    </Panel>
  );
}
